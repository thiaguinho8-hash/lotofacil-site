...
import { formatarDezenas, formatarMoeda } from "@/lib/caixa";
import { getResultadoQuinaPorConcurso, getUltimoResultadoQuina } from "@/lib/quina";
...
export async function generateMetadata({
  ...
  try {
    const resultado = await getResultadoQuinaPorConcurso(numero);
    const dezenas = formatarDezenas(resultado.listaDezenas).join(", ");
    const title = `Resultado Quina Concurso ${resultado.numero} — ${resultado.dataApuracao}`;
    const description = `Resultado da Quina concurso ${resultado.numero}, sorteado em ${resultado.dataApuracao}: ${dezenas}. Veja premiação por faixa e ganhadores.`;

    return {
      title,
      description,
      alternates: { canonical: `/quina/${resultado.numero}` },
...
export default async function ConcursoQuinaPage({
  params,
}: {
  params: Promise<Params>;
}) {
  ...
  let resultado;
  try {
    resultado = await getResultadoQuinaPorConcurso(numero);
  } catch {
    notFound();
  }
  ...
  const jsonLd = {
    ...
    "@type": "Dataset",
    name: `Resultado Quina concurso ${resultado.numero}`,
    description: `Dezenas sorteadas: ${dezenas.join(", ")}. Data do sorteio: ${resultado.dataApuracao}.`,
    datePublished: resultado.dataApuracao,
    url: `${SITE_URL}/quina/${resultado.numero}`,
    creator: {
      "@type": "Organization",
      name: SITE_NAME,
    },
    variableMeasured: "Dezenas sorteadas da Quina",
  };
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8">
      ...
      <h1 className="mb-1 text-2xl font-extrabold sm:text-3xl">
        Resultado Quina — Concurso {resultado.numero}
      </h1>
      ...
      <div className="my-6 flex flex-wrap justify-center gap-3">
        <AffiliateButton />
        <WhatsAppShareButton
          texto={`Resultado da Quina — Concurso ${resultado.numero} (${resultado.dataApuracao}): ${dezenas
            .map((n) => String(n).padStart(2, "0"))
            .join(" - ")}. Confira em ${SITE_URL}/quina/${resultado.numero}`}
        />
      </div>

      <AdSlot id={`ad-quina-concurso-${resultado.numero}`} label="Espaço publicitário" className="my-8 h-24 w-full" />
      ...
      {resultado.listaMunicipioUFGanhadores.length > 0 && (
        <>
          <h2 className="mb-3 mt-8 text-xl font-bold">Onde saíram os ganhadores (5 acertos)</h2>
      ...
      <nav className="mt-10 flex justify-between text-sm font-semibold text-forest dark:text-gold">
        {resultado.numeroConcursoAnterior ? (
          <Link href={`/quina/${resultado.numeroConcursoAnterior}`} className="hover:underline">
        ...
        {!resultado.ultimoConcurso && resultado.numeroConcursoProximo && (
          <Link href={`/quina/${resultado.numeroConcursoProximo}`} className="hover:underline">
